...
CHAPTER_ONE_PATTERN = r"\uc81c\s*1\s*\uc7a5"
CHAPTER_TWO_PATTERN = r"\uc81c\s*2\s*\uc7a5"
CHAPTER_THREE_PATTERN = r"\uc81c\s*3\s*\uc7a5"
...
DIAGRAM_GROUP_REFERENCE_PATTERN = r"[\[\uff3b](" + DIAGRAM_GROUP_PATTERN + r")[\]\uff3d]"
...
def extract_diagram_group_reference(page_content: str) -> str | None:
    """Extract a bracketed diagram group reference such as [차1] from text."""
    match = re.search(DIAGRAM_GROUP_REFERENCE_PATTERN, page_content)
    return match.group(1) if match else None


def _diagram_prefix(diagram_id: str | None) -> str | None:
    return diagram_id[0] if diagram_id else None


def _is_next_diagram_family_text(doc: Document, current_diagram_id: str | None) -> bool:
    """Return True when text appears to announce the next diagram family."""
    referenced_group = extract_diagram_group_reference(doc.page_content)
    if not referenced_group or not current_diagram_id:
        return False
    return _diagram_prefix(referenced_group) != _diagram_prefix(current_diagram_id)


def _chapter_prefix(page_content: str) -> str | None:
    compact_text = re.sub(r"\s+", "", page_content)

    if re.search(CHAPTER_ONE_PATTERN, page_content) and "\ubcf4\ud589\uc790" in compact_text:
        return "\ubcf4"
    if re.search(CHAPTER_TWO_PATTERN, page_content) and "\uc790\ub3d9\ucc28\uc640\uc790\ub3d9\ucc28" in compact_text:
        return "\ucc28"
    if re.search(CHAPTER_THREE_PATTERN, page_content) and "\uc790\uc804\uac70" in compact_text:
        return "\uac70"
    return None


def _is_new_chapter_family(doc: Document, current_diagram_id: str | None) -> bool:
    next_prefix = _chapter_prefix(doc.page_content)
    if not next_prefix or not current_diagram_id:
        return False
    return next_prefix != _diagram_prefix(current_diagram_id)
...
def chunk_documents(docs: list[Document], mapping: IndexMapping) -> list[Document]:
    ...
    for index, doc in enumerate(docs, start=1):
        ...
        try:
            ...
            if _is_new_chapter_family(doc, current_diagram_id):
            ...
            if category in TEXT_CATEGORIES:
                if current_diagram_id:
                    if _is_next_diagram_family_text(doc, current_diagram_id):
                        flushed = flush_text_buffer(text_buffer, current_diagram_id, mapping)
                        if flushed:
                            chunks.append(flushed)
